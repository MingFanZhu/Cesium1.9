 ...
 *                          the contrast value to use for the tile.  The function is executed for every
 *                          frame and for every tile, so it must be fast.
 * @param {Number|Function} [options.invert=0.0] 图层的反色比例，取值在0.0至1.0之间，公式为color=(1.0-invert)*color+invert*(1.0-color)
 * @param {Number|Function} [options.grayscale=0.0] 图层的灰度比例，取值在0.0至1.0之间，公式为color=mix(color,vec3(0.299*R+0.578*G+0.114*B), grayscale)
 * @param {Number|Function} [options.sepia=0.0] 图层的深褐色(怀旧风格)比例，取值在0.0至1.0之间，公式为
 *                          outputRed = (inputRed * .393) + (inputGreen *.769) + (inputBlue * .189)
 *                          outputGreen = (inputRed * .349) + (inputGreen *.686) + (inputBlue * .168)
 *                          outputBlue = (inputRed * .272) + (inputGreen *.534) + (inputBlue * .131)
 *                          color=mix(color,outputcolor, sepia)
 * @param {Number|Function} [options.filterIndex=["gamma","brightness","contrast","hue","invert","grayscale","sepia","saturation"]]
...
function ImageryLayer(imageryProvider, options) {
   ...
   *
   * @type {Array<string>}
   * @default ["gamma","brightness","contrast","hue","invert","grayscale","sepia","saturation"]
   */
   this.filterIndex = defaultValue(
    options.filterIndex,
    defaultValue(
      imageryProvider.defaultFilterIndex,
      ImageryLayer.DEFAULT_FILTERINDEX
    )
  );
  //add by zmf end
  ...
  //add by zmf begin
  /**
   * 反色值，在0.0至1.0，0.0表示不变
   *
   * @type {Number}
   * @default {@link ImageryLayer.DEFAULT_INVERT}
   */
  this.invert = defaultValue(
    options.invert,
    defaultValue(imageryProvider.defaultInvert, ImageryLayer.DEFAULT_INVERT)
  );
  //add by zmf end

  //add by zmf begin
  /**
   * 灰度值，在0.0至1.0，0.0表示不变
   *
   * @type {Number}
   * @default {@link ImageryLayer.DEFAULT_HUE}
   */
  this.grayscale = defaultValue(
    options.grayscale,
    defaultValue(
      imageryProvider.defaultGrayscale,
      ImageryLayer.DEFAULT_GRAYSCALE
    )
  );
  //add by zmf end
  ...
  //add by zmf begin
  /**
   * 深褐色值，在0.0至1.0，0.0表示不变
   *
   * @type {Number}
   * @default {@link ImageryLayer.DEFAULT_HUE}
   */
  this.sepia = defaultValue(
    options.sepia,
    defaultValue(imageryProvider.defaultSepia, ImageryLayer.DEFAULT_SEPIA)
  );
  //add by zmf end
...
//add by zmf begin 反色默认值
/**
 * 反色值
 * @type {Number}
 * @default 0.0
 */
 ImageryLayer.DEFAULT_INVERT = 0.0;
 //add by zmf end
 //add by zmf begin 灰度默认值
 /**
  * 灰度值
  * @type {Number}
  * @default 0.0
  */
 ImageryLayer.DEFAULT_GRAYSCALE = 0.0;
 //add by zmf end
 //add by zmf begin 深褐色默认值
 /**
  * 深褐色值
  * @type {Number}
  * @default 0.0
  */
 ImageryLayer.DEFAULT_SEPIA = 0.0;
 //add by zmf end
