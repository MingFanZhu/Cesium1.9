...
//add by zmf begin
import Filters from "./GlobeFsFilter.js";
//add by zmf end
...
GlobeSurfaceShaderSet.prototype.getShaderProgram = function (options) {
  ...
  //add by zmf begin
  var applyInvert = options.applyInvert;
  var applyGrayscale = options.applyGrayscale;
  var applySepia = options.applySepia;
  //add by zmf end
  ...
  if (
    ...
    //add by zmf begin 宏定义
    if (applyInvert) {
      fs.defines.push("APPLY_INVERT");
    }
    if (applyGrayscale) {
      fs.defines.push("APPLY_GRAYSCALE");
    }
    if (applySepia) {
      fs.defines.push("APPLY_SEPIA");
    }
    //add by zmf end
    if (applySaturation) {
    ...
    for (let i = 0; i < numberOfDayTextures; ++i) {
      ...
      computeDayColor += `\
        ...
        nightBlend,\n\
            //add by zmf begin
            ${applyInvert ? `u_dayTextureInvert[${i}]` : "0.0"},\n\
            ${applyGrayscale ? `u_dayTextureGrayscale[${i}]` : "0.0"},\n\
            ${applySepia ? `u_dayTextureSepia[${i}]` : "0.0"}\
            //add by zmf end
      );\n`;
      if (hasImageryLayerCutout) {
        computeDayColor +=
    ...
    //add by zmf begin
    fs.sources.push(Filters(options.filterIndex));
    //add by zmf end
