...
//add by zmf begin 声明变量
#ifdef APPLY_INVERT
uniform float u_dayTextureInvert[TEXTURE_UNITS];
#endif
//add by zmf end

//add by zmf begin 声明变量
#ifdef APPLY_GRAYSCALE
uniform float u_dayTextureGrayscale[TEXTURE_UNITS];
#endif
//add by zmf end

//add by zmf begin 声明变量
#ifdef APPLY_SEPIA
uniform float u_dayTextureSepia[TEXTURE_UNITS];
#endif
//add by zmf end
...
vec4 sampleAndBlend(
    ...
    float split,
    vec4 colorToAlpha,
    //add by zmf begin 定义改声明，定义在globefsfilter.js中按滤镜顺序生成
    float nightBlend,
    float textureInvert,
